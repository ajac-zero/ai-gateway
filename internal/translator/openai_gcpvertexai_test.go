...
data: {"candidates":[{"content":{"parts":[{"functionCall":{"name":"get_weather","args":{"location":"Paris"}},"thoughtSignature":"dG9vbGNhbGxzaWduYXR1cmU="}]},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":15,"candidatesTokenCount":10,"totalTokenCount":25}}`
	...
	chatCompletionChunks := getChatCompletionResponseChunk(body)
	// The compatibility parser reconstructs the reasoning, tool call, and usage chunks.
	require.Len(t, chatCompletionChunks, 3)

	assert.Contains(t, string(body), `"reasoning_content":"let me think about this."`)

	// Find the tool-call chunk; the parser omits reasoning-only deltas.
	var toolCallChunk *openai.ChatCompletionResponseChunkChoice
	for i := range chatCompletionChunks {
		if len(chatCompletionChunks[i].Choices) > 0 && len(chatCompletionChunks[i].Choices[0].Delta.ToolCalls) > 0 {
			toolCallChunk = &chatCompletionChunks[i].Choices[0]
			break
		}
	}
	require.NotNil(t, toolCallChunk)
	assert.Equal(t, openai.ChatCompletionChoicesFinishReason("tool_calls"), toolCallChunk.FinishReason)
	assert.Equal(t, "get_weather", toolCallChunk.Delta.ToolCalls[0].Function.Name)
	assert.JSONEq(t, `{"location":"Paris"}`, toolCallChunk.Delta.ToolCalls[0].Function.Arguments)

	assert.Contains(t, string(body), `"thinking_blocks":[{"type":"thinking","signature":"dG9vbGNhbGxzaWduYXR1cmU="}]`)

	assert.NotNil(t, chatCompletionChunks[len(chatCompletionChunks)-1].Usage)
