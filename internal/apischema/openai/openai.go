...
// MarshalJSON keeps the OpenAI-compatible reasoning_content field a string while
// preserving provider signatures and redacted reasoning in thinking_blocks.
func (d ChatCompletionResponseChunkChoiceDelta) MarshalJSON() ([]byte, error) {
	thinkingBlocks := d.ThinkingBlocks
	var reasoningContent *string
	if d.ReasoningContent != nil {
		if d.ReasoningContent.Text != "" {
			text := d.ReasoningContent.Text
			reasoningContent = &text
		}
		if d.ReasoningContent.Signature != "" {
			thinkingBlocks = append(thinkingBlocks, ThinkingBlock{Type: "thinking", Signature: d.ReasoningContent.Signature})
		}
		if len(d.ReasoningContent.RedactedContent) > 0 {
			thinkingBlocks = append(thinkingBlocks, ThinkingBlock{Type: "redacted_thinking", Data: base64.StdEncoding.EncodeToString(d.ReasoningContent.RedactedContent)})
		}
	}
	return stdjson.Marshal(struct {
		...
		Content: d.Content, Role: d.Role, ToolCalls: d.ToolCalls, Annotations: d.Annotations,
		ReasoningContent: reasoningContent, ThinkingBlocks: thinkingBlocks,
	})
}

// UnmarshalJSON accepts the standards-compatible string representation emitted
// on the wire as well as the legacy structured representation used internally.
func (d *ChatCompletionResponseChunkChoiceDelta) UnmarshalJSON(data []byte) error {
	type deltaAlias ChatCompletionResponseChunkChoiceDelta
	aux := struct {
		Content          *string                                  `json:"content,omitempty"`
		Role             string                                   `json:"role,omitempty"`
		ToolCalls        []ChatCompletionChunkChoiceDeltaToolCall `json:"tool_calls,omitempty"`
		Annotations      *[]Annotation                            `json:"annotations,omitempty"`
		ReasoningContent stdjson.RawMessage                       `json:"reasoning_content,omitempty"`
		ThinkingBlocks   []ThinkingBlock                          `json:"thinking_blocks,omitempty"`
	}{}
	if err := stdjson.Unmarshal(data, &aux); err != nil {
		return err
	}
	*d = ChatCompletionResponseChunkChoiceDelta{
		Content: aux.Content, Role: aux.Role, ToolCalls: aux.ToolCalls, Annotations: aux.Annotations, ThinkingBlocks: aux.ThinkingBlocks,
	}
	if len(aux.ReasoningContent) == 0 || string(aux.ReasoningContent) == "null" {
		return nil
	}
	var text string
	if err := stdjson.Unmarshal(aux.ReasoningContent, &text); err == nil {
		d.ReasoningContent = &StreamReasoningContent{Text: text}
	} else {
		var structured StreamReasoningContent
		if err := stdjson.Unmarshal(aux.ReasoningContent, &structured); err != nil {
			return err
		}
		d.ReasoningContent = &structured
	}
	for _, block := range aux.ThinkingBlocks {
		if d.ReasoningContent == nil {
			d.ReasoningContent = &StreamReasoningContent{}
		}
		switch block.Type {
		case "thinking":
			if d.ReasoningContent.Signature == "" {
				d.ReasoningContent.Signature = block.Signature
			}
		case "redacted_thinking":
			if decoded, err := base64.StdEncoding.DecodeString(block.Data); err == nil {
				d.ReasoningContent.RedactedContent = decoded
			}
		}
	}
	return nil
