...
func (s GenerateContentEndpointSpec) ParseBody(
	body []byte,
	_ bool,
) (internalapi.OriginalModel, *gcp.NativeGenerateContentRequest, bool, []byte, error) {
	var value any
	...
	req := gcp.NativeGenerateContentRequest{Raw: append([]byte(nil), body...)}
	return s.ModelFromPath, &req, s.Streaming, nil, nil
}

// ParseMultipartBody implements [Spec.ParseMultipartBody].
func (GenerateContentEndpointSpec) ParseMultipartBody([]byte, string, bool) (internalapi.OriginalModel, *gcp.NativeGenerateContentRequest, bool, []byte, error) {
...
func (GenerateContentEndpointSpec) RedactSensitiveInfoFromRequest(
	_ *gcp.NativeGenerateContentRequest,
) (*gcp.NativeGenerateContentRequest, error) {
	return &gcp.NativeGenerateContentRequest{Raw: []byte(`{"contents":"[REDACTED]"}`)}, nil
}
...
type GenerateContentTracer = tracingapi.RequestTracer[gcp.NativeGenerateContentRequest, gcp.GenerateContentResponse, gcp.GenerateContentResponse]
